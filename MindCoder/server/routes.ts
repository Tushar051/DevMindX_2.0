...
//   app.get("/api/collaboration/sessions/:sessionId", authenticateToken, async (req: any, res) => {
//   try {
//     const { sessionId } = req.params;
//     const storage = await getStorage();
//     const session = await storage.db.collection<CollaborationSession>("collaborationSessions")
...
//     if (!session.participants.some((p: { userId: string }) => p.userId === req.user.id)) {
...
//   app.get("/api/collaboration/sessions/:sessionId/messages", authenticateToken, async (req: any, res) => {
//     try {
//       const { sessionId } = req.params;
//       const storage = await getStorage();
//       const messages = await storage.db.collection("collaborationMessages")
//         .find({ sessionId: new ObjectId(sessionId) })
//         .sort({ timestamp: 1 })
//         .toArray();
//       // Verify session exists and user is participant
//       const session = await storage.db.collection<CollaborationSession>("collaborationSessions").findOne({ _id: new ObjectId(sessionId) });
//       if (!session || !session.participants.some(p => p.userId === req.user.id)) {
//         return res.status(403).json({ message: "Unauthorized" });
//       }
//       res.json(messages);
//     } catch (error) {
