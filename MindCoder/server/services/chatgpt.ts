...
export async function generateCodeWithChatGPT(instruction: string, context?: string): Promise<string> {
  try {
    if (!process.env.CHATGPT_API_KEY) throw new Error('CHATGPT_API_KEY is not configured');
...
// --- 3. Chat with AI ---
export async function chatWithChatGPT(message: string, chatHistory?: any[]): Promise<string> {
  try {
    if (!process.env.CHATGPT_API_KEY) throw new Error('CHATGPT_API_KEY is not configured');
